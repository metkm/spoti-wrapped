...
// makes request 3 times wtf
const { data, status, refresh, execute } = await useSpotifyFetch<Pagination<TrackItem>>(`/playlists/${props.id}/tracks`, {
  key: `playlist:${props.id}`,
  params: { offset: offset },
  onResponse: (response) => {
    trackItems.value.push(...response.response._data.items)
...
<template>
  <div class="flex flex-col gap-2 overflow-hidden h-full p-2 rounded-lg">
    ...
    <ul
      ref="container"
      class="h-full overflow-y-auto overflow-fade"
    >
      <li
